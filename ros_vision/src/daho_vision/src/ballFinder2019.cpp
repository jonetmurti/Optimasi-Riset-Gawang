...
void BallFinder::Process(Mat image, Mat field, bool cannyOn, bool checkEllipse)
{
    ...
    for (Point3f kandidat : kandidatBola)
    {
        ...
        int tinggiBoundBox = radius * 2;
        int awalX = center.x - radius * 2;
        int awalY = center.y + radius / 2;
        if (awalX < 0)
        {
            awalX = 0;
            lebarBoundBox += awalX;
        ...
        Rect boundingBox(awalX, awalY, lebarBoundBox, tinggiBoundBox/2);
        Mat maskField = field(boundingBox);
        int countNZero = countNonZero(maskField);
        int countMax = lebarBoundBox * tinggiBoundBox/2;
